...
namespace EPlast.BLL.Services.UserProfiles
{
    public class UserProfileAccessService : IUserProfileAccessService
    {
        ...
        public async Task<bool> CanEditUserProfile(User user, string focusUserId)
        {
            ...
            return
                ((currentUserRoles.Contains(Roles.OkrugaHead) || currentUserRoles.Contains(Roles.OkrugaHeadDeputy)) && await _userService.IsUserInSameCellAsync(currentUser, focusUser, CellType.Region)) ||
                ((currentUserRoles.Contains(Roles.CityHead) || currentUserRoles.Contains(Roles.CityHeadDeputy)) && await _userService.IsUserInSameCellAsync(currentUser, focusUser, CellType.City));
        }
