...
{
    public class UnatstvaMembersStatisticsItemTests
    {
        private readonly UnatstvaMembersStatisticsItem _statisticsItem = new UnatstvaMembersStatisticsItem();

        private readonly IEnumerable<MembersStatistic> _membersStatistics = new List<MembersStatistic>
        {
            new MembersStatistic { NumberOfUnatstvaMembers = 10 },
            new MembersStatistic { NumberOfUnatstvaMembers = 20 }
        };
        ...
        public void GetValue()
        {
            ...
            // Assert
            Assert.Equal(StatisticsItemIndicator.NumberOfUnatstvaMembers, result.Indicator);
            Assert.Equal(_membersStatistics.First().NumberOfUnatstvaMembers, result.Value);
        }
        ...
        public void GetValueList()
        {
            ...
            // Assert
            Assert.Equal(StatisticsItemIndicator.NumberOfUnatstvaMembers, result.Indicator);
            Assert.Equal(_membersStatistics.First().NumberOfUnatstvaMembers + _membersStatistics.Last().NumberOfUnatstvaMembers, result.Value);
        }
